...
def login(driver,email,errorsEncountered): #logs into bing account :) takes account emial and curren error total.
    #go to login page
    driver.get(
...
def getPointsInfo(driver): #gets a whole bunch of stats, and how many timesToSearch on bing
    while True:  # check if profile name is "alfred", the first name of every profile. this means page loaded
        try:
            if driver.find_element_by_id("id_n").text == "alfred":
                break
            else:
                account = driver.find_element_by_id('id_l')  #otherwise, click sign in to immedeatly load alfred. glitch
...
def search(driver,words,timesToSearch): #function to search stuff. requires the list of words and how many times to srch
    while timesToSearch > 0:  # do x number of searches
        ...
        # makes sure page is loaded for next search via profile name 'alfred' showing.
        while True:
            if driver.find_element_by_id("id_n").text == "alfred":
                break
            else:
...
def main():
    ...
    driver = webdriver.Chrome('pathToChromeDriver') #start webdriver
    ...
    for email in f.readlines(): #go through each account, homie
        ...
        print("Signing in...")
        errorsEncountered = login(driver,email,errorsEncountered) #self explanatory
        print("Getting point info...")#get info on how many times to search, and various points stats
        totalPointsEarnable,pointsLeftToEarn,timesToSearch,startingPoints=getPointsInfo(driver) # gets a bunch of info
        ...
        print("Searching...")
        search(driver,words,timesToSearch) # it do be searching tho
